...
Original file is located at
    https://colab.research.google.com/drive/1v__M4x4C4C1myUHq5I-JysVFoB1zLfyN
"""
...
class BiLSTMClassifier(nn.Module):
    ...
    def forward(self, x):
        out, _ = self.bilstm1(x)                       # out shape: [batch, seq_len, 2*hidden_size]
        out = self.dropout1(out)
        out = self.bn1(out[:, -1, :])                  # take last timestep: [batch, 2*hidden_size]

        out_seq = out.unsqueeze(1)                     # shape [batch, 1, 2*hidden_size]
        out, _ = self.bilstm2(out_seq)                 # out shape: [batch, 1, 2*hidden_size]
        out = self.dropout2(out[:, -1, :])             # [batch, 2*hidden_size]
        out = self.bn2(out)
