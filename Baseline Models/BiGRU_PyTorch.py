...
# 5. Reshape for GRU input: (batch, seq_len, input_size)
...
# 8. Define BiGRU model
class BiGRUClassifier(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes):
        super(BiGRUClassifier, self).__init__()
        self.bigru1 = nn.GRU(input_size, hidden_size, batch_first=True, bidirectional=True)
        self.dropout1 = nn.Dropout(0.3)
        self.bigru2 = nn.GRU(2 * hidden_size, hidden_size, batch_first=True, bidirectional=True)
...
model = BiGRUClassifier(input_size=input_size, hidden_size=128, num_classes=num_classes).to(device)
...
history_df.to_csv("bigru_training_history.csv", index=False)
...
plt.legend()
plt.grid(True)
plt.savefig("bigru_accuracy_history.png")
plt.show()
...
plt.tight_layout()
plt.savefig("BiGRU_ROC_Curve.png")
plt.show()
